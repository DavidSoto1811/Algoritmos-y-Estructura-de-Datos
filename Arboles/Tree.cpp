...
private:
	...
	bool _buscar_predecesor_sucesor(Nodo* nodo, t e) {
		if (nodo == nullptr) return false;
		else {
			int r = comparar(nodo->elemento, e);
			if (r == 0) {
				if (nodo->der != nullptr) {
					sucesor = nodo->der->elemento;
				}
				return true;
			}
			else if (r < 0) {
				if (_buscar_predecesor_sucesor(nodo->der, e)) {
					predecesor = nodo->elemento;
				}
				return false;
			}
			else {
				if (_buscar_predecesor_sucesor(nodo->izq, e)) {
					predecesor = nodo->elemento;
				}
				return false;
			}
...
public:
	...
	void Buscar_predecesor_sucesor(t e) { _buscar_predecesor_sucesor(raiz, e); }
	...
	//GEt
	t getpredecesor() { return predecesor; }
	t getsucesor() { return sucesor; }
};